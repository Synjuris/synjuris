...
# LOCAL_MODE: skip login when running locally (SQLite, no REQUIRE_AUTH set).
# Set REQUIRE_AUTH=1 to force login even on localhost.
...
def init_db():
    ...
    # In LOCAL_MODE, ensure a synthetic local user (id=1) always exists
    # so require_auth() can return a valid user_id without login.
    if LOCAL_MODE:
        conn = get_db()
        existing = conn.execute("SELECT id FROM cases LIMIT 1").fetchone()
        # Only seed if the users table exists (SQLite path has no users table)
        try:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS users ("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
...
_LOCAL_USER_ID = 1  # Synthetic user ID used in LOCAL_MODE

def require_auth(handler):
    """Returns user_id or sends 401 and returns None.
    In LOCAL_MODE (SQLite, no REQUIRE_AUTH), always returns _LOCAL_USER_ID.
    """
    if LOCAL_MODE:
...
<div class="card">
  ...
  <div id="signup-form" style="display:none">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
      <div id="role-prose" onclick="selectRole('pro_se')" style="border:2px solid #c9a84c;border-radius:8px;padding:14px;cursor:pointer;text-align:center;background:rgba(201,168,76,0.06)">
        <div style="font-size:22px;margin-bottom:6px">👤</div>
        <div style="font-weight:600;font-size:13px">Representing Myself</div>
        <div style="font-size:11px;color:#6b7a8d;margin-top:3px">Pro se litigant</div>
      </div>
      <div id="role-atty" onclick="selectRole('attorney')" style="border:2px solid #1e3248;border-radius:8px;padding:14px;cursor:pointer;text-align:center">
        <div style="font-size:22px;margin-bottom:6px">⚖️</div>
        <div style="font-weight:600;font-size:13px">Legal Professional</div>
        <div style="font-size:11px;color:#6b7a8d;margin-top:3px">Attorney or paralegal</div>
      </div>
    </div>
...
function selectRole(role){
  ...
  document.getElementById('role-prose').style.border=role==='pro_se'?'2px solid #c9a84c':'2px solid #1e3248';
  document.getElementById('role-prose').style.background=role==='pro_se'?'rgba(201,168,76,0.06)':'';
  document.getElementById('role-atty').style.border=role==='attorney'?'2px solid #c9a84c':'2px solid #1e3248';
...
async function doSignup(){
  ...
  const r=await fetch('/api/signup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pw,tier:role})});
  const d=await r.json();
  if(d.error){showErr(d.error);return;}
  window.location.href='/';
}
...
def call_claude(messages, system="", max_tokens=2000, model="claude-sonnet-4-20250514"):
    ...
    try:
        ...
        return f"⚠️ Could not reach Anthropic. Check your internet connection.\n\nDetail: {e.reason}"
    except TimeoutError:
        return "⚠️ Request timed out. The response was taking too long — try again."
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/chat":
            ...
            reply = reply + "\n\n---\n*SynJuris provides legal information and organizational tools only — not legal advice. This output does not constitute legal counsel and does not create an attorney-client relationship. This information is protected speech under the First Amendment. Always consult a licensed attorney before filing any document with a court. SynJuris is not a law firm.*"
...
function appendMsg(role,content,temp=false){
  ...
  if(role==='user'){
    el.style.cssText='background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:10px 14px;font-size:13px';
    ...
    el.style.cssText='background:var(--surface2);border:1px solid var(--border);border-radius:var(--r);padding:14px 16px;font-size:13px';
    el.innerHTML=`<div style="font-size:10px;text-transform:uppercase;letter-spacing:.1em;color:var(--gold);margin-bottom:8px">Search Results</div><div class="mb" style="white-space:pre-wrap;line-height:1.7;color:var(--ink)">${esc(content)}</div><div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border);font-size:11px;color:var(--ink3)">This is general legal information. You must determine whether and how it applies to your situation. <strong>SynJuris does not provide legal advice.</strong></div>`;
  }
  w.appendChild(el);
...
async function renderArguments(c){
  ...
  <div style="max-width:680px">
    ...
    <div class="notice n-info" style="margin-bottom:16px">
      <strong>How this works:</strong> SynJuris groups your confirmed evidence by legal category and surfaces the relevant statute. You review the groupings and decide what to present in court.
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px">
      <div class="card" style="padding:14px 16px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <button class="btn btn-p" id="theory-btn" onclick="buildTheory()" disabled style="white-space:nowrap;margin-left:12px">Build</button>
        </div>
      </div>
      <div class="card" style="padding:14px 16px">
        <div style="display:flex;justify-content:space-between;align-items:center">
...
if __name__ == "__main__":
    ...
    print(f"  Auth mode: {'⚠ LOCAL (no login required)' if LOCAL_MODE else '✓ Login required'}")
    if not API_KEY:
